...
(function rhsTreeMenu() {
// initialise global variables outside functions
let observer = new IntersectionObserver(handler, { threshold: [0] });
let selection;
let headings = [...document.querySelectorAll("#content-column h2, #content-column h3")];
let rhsToc = [...document.querySelectorAll("ul.right-sidebar-nav a")];
let a = null;
...
function handler(entries) {
    ...
    if (firstVisibleId === null & a!= null){
        ...
        if (scrollDirection == "down"){
            ...
            const current = document.querySelector(`#right-sidebar-outline > ul li.selected`);
            if (current) {
            ...
            if(previousByClass(a.parentElement, "rhs-toc-entry") == null){
                parent_by_selector(a.parentElement, "ul:not(.right-sidebar-nav)") ? parent_by_selector(a.parentElement, "ul:not(.right-sidebar-nav)").parentElement.classList.add("selected") : null;
            } else {
                previousByClass(a.parentElement, "rhs-toc-entry") ? previousByClass(a.parentElement, "rhs-toc-entry").classList.add("selected") : null;
            }
        }
        return;
    }
...
items = document.querySelectorAll(`.right-sidebar-nav a`);
...
let rhsTreeMenu;
document.addEventListener('DOMContentLoaded', rhsTreeMenu);